function [sc, sov_tsmd, spm_tsmd, som_tsmd, S1_tsmd, S2_tsmd, V1_tsmd, V2_tsmd] = tsmd2(Y, delY, r, MeanSubtraction)
% ------------------------------------------------------------------------
...
% Modified on 2/22/2022 - modified help documentation
% ------------------------------------------------------------------------
...
[Ut, St, ~] = svd([Y,delY], 'econ');

% --------- 2.TSVD-BASED TEMPORAL DIMENSIONALITY REDUCTION ---------------

Q = Ut(:, 1:r);  % Orthogonal basis by truncating POCs 
Yr = Q'*Y;       % Temporally-reduced measurement matrix
delYr = Q'*delY; % Temporally-reduced gradient matrix
