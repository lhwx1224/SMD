function [M, K, T, Lambda, Phi, Phi_tilde, fw, omega_n] = AMM_uniform_beam(Nam, dx, m0, EI0, L0, EIx_str, mx_str, Fx_str, FastGen)
...
if isequal(lower(FastGen),'true')
    ...
    root_temp = load('L:\My Drive\Graduate study\Research\Projects\Output-only Modal Analysis Toolbox\DATA\AMM\Uniform beam\roots_318.mat');
    root = root_temp.root;
else
...
disp(['Total number of modes solved/included:', num2str(Nam)])
...
if isequal(lower(FastGen),'true')
    Y_norm_temp = load('L:\My Drive\Graduate study\Research\Projects\Output-only Modal Analysis Toolbox\DATA\AMM\Uniform beam\Y_norm_318.mat');
    Y_norm = Y_norm_temp.Y_norm;
else
    Y_norm = sqrt(double(int(Y.^2,x,[0 L0]))); % ANALYTICAL MAGNITUDE
end
% ------------------- Numerical Integration ------------------------------
% Y_norm = zeros(Nam, 1);
% for i = 1:Nam
%     fun_Ysqr = matlabFunction(Y(i).^2);
%     Y_norm(i) = integral(fun_Ysqr, 0, 1, "AbsTol", 1e-3);
%     progress_bar(i, Nam, 'Calculating Mode Amplitude')
% end
...
% ----------------- Numerical Integration -------------------------------
% % Parallelized computation when possible
...
%         progress_bar(count, Nam*(Nam - 1)/2 + Nam, 'Calculating Stiffness Matrix')
%         count = count + 1;
%     end
% end
% 
...
%         progress_bar(count, Nam*(Nam - 1)/2 + Nam, 'Calculating Mass Matrix')
%         count = count + 1;
%     end
% end
% 
...
% Test Code: using symbolic to reduce decomposition error
% Ms = sym(M);
% Ks = sym(K);
...
disp("====================== BEAM SPECIFICATION =============================================")
disp(['Flexural Rigidity:', EIx_str])
disp(['Mass density:', mx_str])
disp(['Force Location:', Fx_str])
disp(['Spatial Resolution (dx):', num2str(dx)])

disp("====================== DISCRETIZATION INFORMATION =====================================")

format("short")
disp(['Time elapsed (sec):', num2str(time_taken)])

disp('The stiffness matrix (first 10-by-10):')
K(1:10,1:10)
disp('The mass matrix (first 10-by-10):')
M(1:10,1:10)
