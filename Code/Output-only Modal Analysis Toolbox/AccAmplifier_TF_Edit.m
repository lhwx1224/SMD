function [yamp_test, FRFq] = AccAmplifier_TF_Edit(y, y_test, CHref, CHamp, fs,...
    fc, FRF_scale, Nf, windsize, deadband, DeadBandMode, CHbyCHAmp)
...
% 
% Syntax: [yamp, yamp_FRF] = AccAmplifier_TF(y, y_test, CHref, fs, fc,
...
% Assign new variable which dictates the FRF esimates
if isempty(CHbyCHAmp)
    CH_FRFs = CHamp;
    NFRFs = length(CH_FRFs); % Number of FRF estimates
elseif isnumeric(CHbyCHAmp) && length(CHbyCHAmp) == 1
    CH_FRFs = CHbyCHAmp;   
    NFRFs = length(CH_FRFs); % Number of FRF estimates
else 
    error('Check the CHbyCHAmp input!')
end

% ------------------------ IDENTIFY TIME VECTORS -------------------------
disp('---------- CHECK TIME VECTORS -----------')
...
FRF = zeros(length(fspan_frf),NFRFs);  % Nf*# of CHamp freq. vector (for FRF estimation)  
FRFq = zeros(TransformSize/2, NFRFs);  % # of Half freq*# of CHamp new query points for full-size transformation
FRFts = zeros(TransformSize, NFRFs);   % # of Full freq*# of CHamp new query points for IFFT

% ------------------------------------------------------------------------
%                   FRF ESTIMATION SECTION BEGINS
...
disp('Pre-amplification is in use! FRF estimation Begins!')
    ...
    % ---------------- 10/6/2021 MOD ---------------------------
    if ~isempty(deadband)
        disp(' ----------- CHANNEL: ', num2str(CH_FRFs(i)), ' -------------')
        disp(['Deadband is applied from 0-',num2str(deadband(1)),' and from ',num2str(deadband(2)),'-',num2str(fc),'.'])
        % Deadband assignment
...
% ------------------------------------------------------------------------
disp('---------- FRF AMPLIFICATION BEGINS ----------')
disp(['Applying the amplifier to CHs:', num2str(CHamp), '.'])
...
if ~isempty(CHbyCHAmp) % IF not channel-by-channel amplification
    for i = 1:size(Yt, 2)
        % --------- The calibration data set ------
        Ynamp(:,i) = Yn(:,i).*abs(FRFtsc(:,1)); % Amplify the signals according to only one specified channel
...
else                  % IF channel-by-channel amplification
% --------------- MOD 10/25/21 Ends here -------------
    for i = 1:size(Yt, 2)
        % --------- The calibration data set ------
        Ynamp(:,i) = Yn(:,i).*abs(FRFtsc(:,i));       % Amplify channel-by-channel       
        ynamp(:,i) = real(ifft(Ynamp(:,i))); 
        % --------- The modal test data set -------
