...
nm = 178;                             % Number of modes to consider
...
L0 = 1;                              % Length of the beam
m0 = 1000;                              % Mass density 
mx = 'm*(L - 0.5*x)/L';              % Mass distribution
M0 = 0;                              % Lumped mass
k0 = 0;                              % Lumped stiffness
EI0 = 0.1;                             % Flexural rigidity
EIx = 'EI*(L - 0.5*x)/L';                % Flexural rigidity distribution
if isequal(lower(ForceWeight), 'beam')
    Fx = strcat('dirac(x - L*',num2str(ExciteLoc),'/10)');
elseif iseuqal(lower(ForceWeight), 'base')
    Fx = '1';
end
% 
dx = 0.0059*L0;                        % Sampling spatial distance
x = 0:dx:L0;                          % Define domain of the beam
p = length(x);
% SIMULATION TIME SETUP
dt = 0.001;                        % sampling time (Default for nm == 10 is 0.0025)
fs = 1/dt;                          % sampling frequency
tend = 120;                           % ending time of simulation
...
[M_am, K_am, T, Lambda, phi_am, phi_tilde, weight, omega_am] = AMM_uniform_pp_beam(nm, dx, m0, EI0, L0, EIx, mx, 'dirac(x - L/2)');
...
figure(3), set(gcf,'papersize',[6 6*1.5]), set(gcf,'paperposition',[0 0 6 6*1.5])
print([cd,'\Discretization_info.png'],'-dpng', '-r600')
    ...
    Lambda = Data.Lambda;
    phi_am = Data.phi_am;
    phi_tilde = Data.phi_tilde;
    weight = Data.weight;
    omega_am = Data.omegan;
    tend = Data.tend;
    fs = Data.fs;
    dt = Data.dt;
    dx = Data.SpatialResolution;
    L0 = Data.Dimensions(1);
    rr = Data.rr;
    % Number of modes for the discretized model
    nm = 178;                             % Number of modes to consider

    Forcing = Data.ForcingType;
    Method = Data.SimulationMethod;
    SimSpace = Data.SimulationSpace;
    ExciteForm = Data.ExciteForm;
    ForceWeight = Data.ForceWeight;
    DampingType = 'Modal';
    ExciteLoc = 10; % Excitation location
    t = 0:dt:tend;                      % Time vector
    x = 0:dx:L0;                          % Define domain of the beam
...
Phi_am = phi_am(2:end-1,:); % Normalized mode shapes without zeros
Phi_tilde = phi_tilde(2:end-1,:);
...
% Normalize the approximated modes
Phi_am_temp = Phi_tilde*U_am(1:n, 1:1:end);
norm_Phi_x = zeros(2*n, 1);
% Phi_tilde_n = zeros(size(Phi_tilde));
for i = 1:2*n
    norm_Phi_x(i) = norm(Phi_am_temp(:,i));
%     Phi_tilde_n(:,i) = Phi_tilde(:,i)/norm_Phi_x(i);
end
...
%% RUN BEAM SIMULATIONS
% ------------------------------------------------------------------------
%                  BEAM SIMULATION STARTS HERE
% ------------------------------------------------------------------------
close all
...
% IW_temp = load('IW0.mat');
% IW = IW_temp.IW_normalized;
% IW = linspace(1,10000,nm);
% IW = 1./norm_Phi_x(1:2:end)';
% Empirical Modal Contribution
% IW = 0.000897*(1:178).^-1.798;
IW = ones(1,nm);
IWindx1 = 1:2:2*nm;
IWindx2 = 2:2:2*nm;
...
[~, I] = sort(IW_temp(1,:));
IW = IW_temp(:,I);
% IW(2,1:6) = [0 0 0 0 0 0];
% IW(2,1:6) = [1.5e-2 1.5e-2 1e-2 1e-2 1e-2 1e-2];
IW(2,1:6) = [1.2e-2 1.2e-2 1e-2 1e-2 1e-2 1e-2];

% MODES INCLUDED
ModePool = [1:2:1000; 2:2:1000];
ModePool = [1:1:size(ModePool,2); ModePool];
ModeIndices = [1:nm];
IncModes = reshape(ModePool(2:3,ModeIndices),[],1);
IncModes = sort(IncModes, 'ascend');
...
subplot(311)
yyaxis left
plot(real(Phi_x(:,ModeIndices))*diag(IW(2,1:2:end))), hold on
ylabel('$\phi_{0i}$')
yyaxis right
plot((real(y_am(1,:))))
...
pbaspect([2.5 1 1])
subplot(312)
plot(var(eta_am(:,1:2:end)),'o')
set(gca,'yscale','log')
ylabel('Initial modal amplitude')
xlabel('Mode Index')
pbaspect([2.5 1 1])
subplot(313)
imagesc(real(y_am'))
% mycolorbar();set(colorbar,'visible','off');
xlabel('Time Samples')
ylabel('Spatial Samples')
pbaspect([2.5 1 1])
set(gcf,'papersize',[6 6*1.5])
set(gcf,'paperposition', [0 0 6  6*1.5])
print('ICnField_pp_beam_1.png','-dpng','-r600')
...
fprintf(fid,"---------------------- PIN-PINED BEAM SPECIFICATION ---------------------------------------------\n");
fprintf(fid,['Size of the beam (L0): (',num2str(L0),')', '\n']);
fprintf(fid,['Mass Density Distribution: ', mx, '\n']);
fprintf(fid,['Flexural Rigidity Distribution: ', EIx, '\n']);
fprintf(fid,['Force Specification: ', Fx, '\n']);
fprintf(fid,['Mass Density Constant (m0): ', num2str(m0), '\n']);
fprintf(fid,['Flexural Rigidity Constant (EI0): ', num2str(EI0), '\r\n']);
...
fprintf(fid,['Spatial Resolution (dx): (', num2str(dx),')']);
